...
| PyTorch image, Jupyter-driven ModelScope install | `runpod/pytorch:1.0.2-cu1281-torch280-ubuntu2404` | `zai-org/GLM-5.2` | Jupyter became reachable when `JUPYTER_PASSWORD` was set. `pip install modelscope` worked and `modelscope download --model zai-org/GLM-5.2 --local_dir /root/.cache/modelscope/zai-org/GLM-5.2` began downloading shards. Stopped at about 29 GB after confirming `config.json` reports `dtype: bfloat16` and no quantization config. Deleted to avoid burning H200 time on a likely-unloadable 1.5 TB BF16 repo. |
...
ModelScope remains the correct Z.AI-recommended alternative source for `zai-org/GLM-5.2`, and the install/download mechanics are now verified on RunPod through Jupyter. However, the documented ModelScope artifact is BF16 by config and has no quantization config. For the requested FP8 H200 test, use Hugging Face `zai-org/GLM-5.2-FP8` with an `HF_TOKEN`, or obtain an explicit FP8 ModelScope revision/path from Z.AI before retrying ModelScope.
